...
日轮与山形曲线位于整页底层，记录库透过玻璃呈现；观看时使用播放器底栏入口与底部快速记录窗，不离开视频。默认深色，支持手动切换浅色、临时拖动调整位置、窗口缩放、键盘操作和草稿恢复。
...
`npm test` 目前包含 67 项本地数据、后台、数据库、Notion、播放器定位与帮助交互检查。界面通过本地预览检查；这不等于所有视频网站、Google 登录、线上云同步或 Notion 导出均通过实际扩展验收。当前没有在此仓库声明未核实的用户规模或线上效果。
