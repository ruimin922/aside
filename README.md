...
一个 Chrome 扩展 + 渐进式 Web 应用（PWA）组合，看访谈、课程、视频播客或电影时一键捕捉视频时间、留下想法，支持 Markdown / Notion 导出与多端云同步。
...
- 完整标签页：可点击悬浮主页的「旁白」品牌名打开；浏览器受限页面回退到同一浏览器窗口内的标签页，不再弹出独立小窗
- 搜索：搜索视频标题、正文和标签，直接展示命中片段、高亮关键词，定位具体笔记或回到视频时刻
- 同步：Google 账号一键登录，基于 Supabase 的多端同步
- 导出：单个视频或全部笔记 → Markdown / Notion
...
- **上传 Chrome 应用商店**：下载 `aside-v版本-chrome-web-store.zip`，在已有插件条目中上传。商店包会移除仅供本地固定扩展 ID 的 `key` 字段。
- **本次改动与安装说明**：[1.9.12](docs/RELEASE-1.9.12.md)。本地包由 `npm run package` 生成；GitHub Releases 中的版本以实际发布为准。
...
`npm test` 目前包含 85 项本地数据、后台、数据库、Notion、播放器定位、同步反馈、精确搜索与帮助交互检查。界面通过本地预览检查；这不等于所有视频网站、Google 登录、线上云同步或 Notion 导出均通过实际扩展验收。当前没有在此仓库声明未核实的用户规模或线上效果。
